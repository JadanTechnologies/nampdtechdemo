    ...
    return (
        <div>
            ...
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold text-dark dark:text-gray-100">Communication Templates</h1>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg">
                {loading ? <div className="flex justify-center items-center h-64"><Spinner size="lg" /></div> : (
                     <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                {templates.map(template => (
                                    <tr key={template.id} className="bg-white dark:bg-gray-800 border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        {templates.length === 0 && <p className="text-center py-8 text-gray-500 dark:text-gray-400">No templates created yet.</p>}
                    </div>
                )}
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col">
                <form onSubmit={handleSubmit} className="flex flex-col h-full">
                    <div className="p-6 border-b dark:border-gray-700">
                        <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100">{template.id ? 'Edit Template' : 'Create New Template'}</h3>
                    </div>
                    <div className="p-6 space-y-4 overflow-y-auto">
                        <div>
                         ...
                         <div>
                            <label htmlFor="type" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Template Type</label>
                            <select id="type" value={formData.type} onChange={e => setFormData(p => ({ ...p, type: e.target.value as 'sms' | 'email' }))} className="mt-1 block w-full border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-white rounded-md shadow-sm">
                        ...
                        <div>
                            ...
                            <textarea id="content" value={formData.content} onChange={e => setFormData(p => ({ ...p, content: e.target.value }))} rows={10} className="mt-1 block w-full border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-white rounded-md shadow-sm" />
                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Available placeholders: `{"{{fullName}}"}`, `{"{{email}}"}`, `{"{{memberId}}"}`.</p>
                        </div>
                    </div>
